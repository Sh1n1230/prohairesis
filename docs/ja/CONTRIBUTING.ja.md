...
- **KISS。** 環境については、これは「エージェントが知らねばならない表面を小さく保つ」ことを意味する。エージェントが覚えるべきコマンドは 1 つ増えるごとに context window への課税であり、誤解されうるものが 1 つ増えることでもある。P2 はそれを 1 つも増やさない。`report`・`metrics`・`hooks` はすべて人間向けである。
